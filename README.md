...
**Индексы** применяются для оптимизации поиска (`idx_logs_detected_at`), а также для упрощения удаления последних логов. А ещё у меня много ограничения типа UNIQUE(их БД автоматически делает индексами, так что я думаю мне ондного явно объявленного хватит)
![Схема базы двнных](images/1.png)
---

## 3. SQL-код создания базы данных и таблиц
P.S. я объяснял всё в коментариях а не тут перед кодом
